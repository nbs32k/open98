...
VOID insw( USHORT port, VOID *addr, UINT count )
{
	asm volatile ( "rep insw" : "+D" ( addr ), "+c" ( count ) : "d" ( port ) : "memory" );
}

VOID outsw( USHORT port, VOID *addr, UINT count )
{
...
)
{
	...
	UCHAR status = inb( ATA_PRIMARY_COMM_REGSTAT );

	DbgPrintFmt( "Waiting for status.\n" );
	while ( status & STAT_BSY )
	{
		UINT i = 0;
		while ( 1 )
		{
			DbgPrintFmt( "Printing stuff %d\n", i );
			i++;
		}
		for ( i = 0; i < 0x0FFFFFFF; i++ )
		{
		}
		DbgPrintFmt( "Checking regstat.\n" );
		status = inb( ATA_PRIMARY_COMM_REGSTAT );
	}

	if ( status == 0 ) return 0;

	DbgPrintFmt( "Status indicates presence of a drive. Polling while STAT_BSY... " );
	while ( status & STAT_BSY )
	{
		DbgPrintFmt( "\ninb(ATA_PRIMARY_COMM_REGSTAT);... " );
		status = inb( ATA_PRIMARY_COMM_REGSTAT );
	...
	UCHAR mid = inb( ATA_PRIMARY_LBA_MID );
	UCHAR hi = inb( ATA_PRIMARY_LBA_HI );
	if ( mid || hi )
	{
	...
	DbgPrintFmt( "Waiting for ERR or DRQ.\n" );
	// Wait for ERR or DRQ
 ...
 *     Transfer 256 16-bit values, a USHORT at a time, into your buffer from I/O port 0x1F0. (In assembler, REP INSW works well for this.)
...
)
{
	...
	for ( i = 0; i < sectorcount; i++ )
	{
		...
		while ( 1 )
		{
			...
			{
				DbgPrintFmt( "DISK SET ERROR STATUS!" );
			}
		}
		// Transfer the data!
