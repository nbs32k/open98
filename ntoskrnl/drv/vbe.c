...
static INT iPrivateX = 8;
static INT iPrivateY = 16;
	...
	for ( INT iy = uiY; uiY + 16 > iy; iy++ )
	{
		for ( INT ix = uiX; uiX + 8 > ix; ix++ )
		{
				   ...
				   ix >= 0 && iy >= 0 && ix < KiVBEData.Width && iy < KiVBEData.Height )
			{
				...
				ulOffset = ( ULONG64 )( ( ix * 4 ) + ( iy * KiVBEData.Pitch ) );
				*( INT* )( KiVBEData.Address + ulOffset ) = uiColor;
...
)
{
	...
	ulOffset = ( ULONG64 )( ( uiX * 4 ) + ( uiY * KiVBEData.Pitch ) );
	return *( INT* )( KiVBEData.Address + ulOffset );
}
...
)
{
	...
	Framebuffer = KiGetStivale2Tag( stivale2_struct, STIVALE2_STRUCT_TAG_FRAMEBUFFER_ID );


	//KiVBEData -> NT FrameBuffer Struct
	KiVBEData.Address = Framebuffer->framebuffer_addr;
	KiVBEData.Width = Framebuffer->framebuffer_width;
	KiVBEData.Height = Framebuffer->framebuffer_height;
	KiVBEData.Pitch = Framebuffer->framebuffer_pitch;
}
