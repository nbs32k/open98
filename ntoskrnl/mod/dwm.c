...
)
{
	...
	CHAR* tempTitle = ( CHAR* )kcalloc( RtlStringLength( Title ) + 1 );
	UINT* Framebuffer = kcalloc( ( Width - X ) * ( Height - Y ) * sizeof( UINT ) );

	RtlSetMemory( Framebuffer, 0x000000, sizeof( Framebuffer ) );
...
)
{
	...
	KSYSTEM_WINDOW window = DwmAcquireInformation( WindowID );
...
)
{
	...
	for ( INT tempI = 0; tempI < MAX_WINDOWS; ++tempI )
	{
		...
		KSYSTEM_WINDOW Window = DwmAcquireInformation( tempI );
