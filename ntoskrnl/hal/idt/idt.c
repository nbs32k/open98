...
{
	DbgPrintFmt( "Raise Exception: %s\nrip: %p rax: %p\nrbx: %p rcx: %p\nrdx: %p rsi: %p\nrdi: %p rbp: %p", exception_messages[ regs->int_no ], regs->rip,
	regs->rax, regs->rbx, regs->rcx, regs->rsi, regs->rdi, regs->rbp ); //autism 
...
{
	if ( interrupt_handlers[ Register->int_no ] != 0 )
	{
		KiInterruptHandler handler = interrupt_handlers[ Register->int_no ];
		handler( Register );
	}
